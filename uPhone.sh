...
clear
echo -e "${PURPLE}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓${NC}"
echo -e "${PURPLE}┃${NC}          ${W}UGPHONE AUTO BUYER PRO (FIXED)${NC}            ${PURPLE}┃${NC}"
echo -e "${PURPLE}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
...
echo -e "  ${GREY}1.${NC} Nhật (JP)  ${GREY}2.${NC} Sing (SG)  ${GREY}3.${NC} Mỹ (US)  ${GREY}4.${NC} Đức (DE)  ${GREY}5.${NC} Hồng Kông (HK)"
read -p "  ╰─> Nhập số: " CH
case $CH in 
...
# Fix lỗi JQ Parse: Kiểm tra dữ liệu trước khi parse
JSON_NODES=$(curl -s "https://onionoo.torproject.org/details?search=country:$CC")
NODES=$(echo "$JSON_NODES" | jq -r '.relays[] | select(.running==true) | .fingerprint' 2>/dev/null | shuf -n 20 | tr '\n' ',' | sed 's/,$//')

echo -e "DataDirectory $PREFIX/var/lib/tor\nLog notice stdout\nSocksPort 9050" > "$TORRC"
if [[ -n "$NODES" && "$NODES" != "null" ]]; then
    echo -e "ExitNodes $NODES\nStrictNodes 1" >> "$TORRC"
else
    echo -e "ExitNodes {$CC}\nStrictNodes 1" >> "$TORRC"
fi

is_ready=false
# Chạy tor và đọc log để hiển thị tiến trình
while read -r line; do
    if [[ "$line" == *"Bootstrapped"* ]]; then
        percent=$(echo "$line" | grep -oP "\d+%" | head -1 | tr -d '%')
        [[ -n "$percent" ]] && render_bar "Tiến trình Tor" "$percent"
...
if [ "$is_ready" = true ]; then
    ...
    if [ -n "$AMT" ]; then 
        ...
        # Fix lỗi syntax dấu ngoặc ở đây
        if [[ -n "$ORD" ]]; then
...
clear
echo -e "\n\033[1;32m✅ HỆ THỐNG ĐÃ FIX LỖI & THÊM HỒNG KÔNG!\033[0m"
echo -e "\033[1;37mNhập lệnh để bắt đầu:\033[0m \033[1;36mbuy\033[0m\n"
