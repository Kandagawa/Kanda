...
select_country() {
    ...
    while true; do
        ...
        if [[ "$clean_input" == "all" ]]; then
            ...
            printf "      ${GREY}⏳ Đang kiểm tra IP cho ${YELLOW}${country_code^^}${GREY}...${NC}"
            country_nodes=$(curl -s "https://onionoo.torproject.org/summary?search=country:$country_code&running=true" | jq '.relays | length // 0' 2>/dev/null)
            ...
            if [[ "$country_nodes" -lt 3 ]]; then
                echo -e "      ${RED}✗ Quốc gia ${YELLOW}${country_code^^}${RED} chỉ có ${YELLOW}${country_nodes}${RED} IP (Quá ít). Vui lòng chọn quốc gia khác!${NC}"
            else
                display_country="${country_code^^}"
                echo -e "      ${GREEN}✓ Quốc gia ${YELLOW}${country_code^^}${GREEN} có ${YELLOW}${country_nodes}${GREEN} IP đang hoạt động.${NC}"
                break
            fi
        else
            echo -e "      ${RED}✗ Mã không hợp lệ! Vui lòng nhập lại.${NC}"
...
run_tor() {
    ...
    if [ "$is_ready" = true ]; then
        ...
        while [[ "$stop_flag" == "false" ]]; do
            ...
            echo -e "\n  ${GREY}» ${YELLOW}[Nhấn R]${GREY} : Chọn lại cấu hình${NC}"
            echo -e "  ${GREY}» ${YELLOW}[Nhấn X]${GREY} : Xoay IP ngay lập tức${NC}"
            echo -e "  ${GREY}» ${YELLOW}[Nhấn E]${GREY} : Thoát khỏi ứng dụng${NC}\n"
            ...
            if [[ "$key" == "r" || "$key" == "R" ]]; then
                ...
                cleanup
                clear
                echo -e "\n  ${RED}❌ Đã thoát hệ thống. Tạm biệt!${NC}\n"
                exit 0
            fi
...
main() {
    ...
    while true; do
        ...
        printf "\n  ${PURPLE}◈${NC} ${GREEN}Tổng số IP đang hoạt động:${NC} ${YELLOW}${total_nodes}${NC} ${GREY}IPs${NC}\n"
